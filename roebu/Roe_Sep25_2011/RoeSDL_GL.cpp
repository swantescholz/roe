...
namespace roe {
	...
	void SDL_GL::init(int width, int height, std::string sWindow, std::string sTab, std::string sIcon, flag32 flags) {
		...
		if( SDL_Init( SDL_INIT_VIDEO | SDL_INIT_TIMER | SDL_INIT_AUDIO) < 0 ) {
			...
			roe_except(std::string("Video initialization failed: ") + SDL_GetError(), "init");
		}
		...
		if( !info ) {
			...
			roe_except(std::string("Video query failed: ") + SDL_GetError(), "init");
		}
		...
		if( m_pScreen == nullptr ) {
			roe_except(std::string("Video mode set failed: ") + SDL_GetError(), "init");
		}
		SDL_WM_SetCaption(sWindow.c_str(), sTab.c_str());//titel
		
		// **SDL_ttf**
		if (TTF_Init() != 0) {
			roe_except(std::string("TTF_Init() failed: ") + TTF_GetError(), "init");
		}
		...
		glPrimitiveRestartIndex(roe_pri);
	...
	// *gl*
	Matrix SDL_GL::applyCameraToScene(const Camera& c) {
		//new (without gluLookAt)
		Matrix camMat = c.getCameraMatrix();
		glMultMatrixf(camMat.n);
		return camMat;
	}
	...
	void SDL_GL::toggleFullscreen() {
		...
		if(m_pScreen == nullptr) m_pScreen = SDL_SetVideoMode(w, h, bitspp, flags); //If toggle FullScreen failed, then switch back
		if(m_pScreen == nullptr) roe_except("toggle to/from fullscreen failed", "toggleFullscreen");
	}
	...
	void SDL_GL::setGLViewport(int w, int h) {
		...
		double zNear = 0.01, zFar = 65536.0, fovx = 80.0; //you may change this values (std: 1.0, 65536.0, 60.0)
		
		right  = zNear * tan(ROE_DEG_TO_RAD(0.5*fovx));
		left   = -right;
		top    = right/ratio;
		bottom = -top;
		...
		glPopMatrix();
		m_mProjection = Matrix::projection(ROE_DEG_TO_RAD(fovx), ratio, zNear, zFar);
		glLoadMatrixf(m_mProjection.n);
		...
		if (sIcon.length() > 3) {
			if (util::stringEndsWith(sIcon, ".bmp")) {
				...
				if ((int)in1 % 8 != 0 || (int)in1 < 24 || (int)in1 > 32) {
					roe_except("tga icon '" + sIcon + "' has bad bits per pixel: " + util::toString((int)in1), "setIcon");
				}
