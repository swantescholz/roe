...
namespace roe {
	...
	public:
		...
		Matrix applyCameraToScene(const Camera& c); //cameras operations to scene (BEFORE the scene is rendered, before lighting), returns applied camera matrix
		Matrix getProjectionMatrix() {return m_mProjection;} //returns the projection matrix
		void renderSkyBox(const Vector3& size, const Vector3& center, const std::string& name, //call BEFORE other renderings
		                  ...
		                  const std::string& left  = "_lt", const std::string& right = "_rt",
		                  const std::string& front = "_ft", const std::string& back  = "_bk", const Color& color = Color(1,1,1,1));
	...
	private:
		...
		//membervariables
		Matrix m_mProjection; //projection matrix
