...
namespace roe {
	...
	public:
		...
		~Matrix() {};
#ifdef ROE_COMPILE_PHYSICS
		Matrix(const btMatrix3x3& m)
			: m14(0), m24(0), m34(0)
			, m41(0), m42(0), m43(0), m44(1)
		{
			btVector3 row;
			row = m.getRow(0); m11 = row.x(); m21 = row.y(); m31 = row.z();
			row = m.getRow(1); m12 = row.x(); m22 = row.y(); m32 = row.z();
			row = m.getRow(2); m13 = row.x(); m23 = row.y(); m33 = row.z();
		}
		Matrix& operator=(const btMatrix3x3& m) {
			m14 = m24 = m34 = 0;
			m41 = m42 = m43 = 0; m44 = 1;
			btVector3 row;
			row = m.getRow(0); m11 = row.x(); m21 = row.y(); m31 = row.z();
		...
		operator btMatrix3x3() const {return btMatrix3x3(m11,m12,13,m21,m22,m23,m31,m32,m33);}
		btMatrix3x3 bt() const {return btMatrix3x3(m11,m12,13,m21,m22,m23,m31,m32,m33);}
#endif
		...
		Matrix toRotationMatrix() const; //returns the matrix with '1' in the lower-right and '0's at the right and at the bottom
		...
		static Matrix translation   (const float x, const float y, const float z) {return translation(Vector3(x,y,z));}
		//all rotations work CW if you look into positive vector direction and CCW if you look into negative vector direction
		...
		static Matrix rotationQuaternion(const float x, const float y, const float z, const float w); //transforms a quaternion to a rotation matrix
		...
		static Matrix camera        (const Vector3& vPos, const Vector3& vDir, const Vector3& vUp  = Vector3(0.0f, 1.0f, 0.0f));
		static Matrix toTex2DMatrix (const Matrix& m);
		static float  projectedDistance  (const Vector3& a, const Vector3& b, const Matrix& m);
		static float  projectedDistanceSq(const Vector3& a, const Vector3& b, const Matrix& m);
		static float  projectedDistanceY (const Vector3& a, const Vector3& b, const Matrix& m);
		static Matrix normal (const Matrix& m); //returns the the upper-left corner of the transformation matrix m first inverted and then transposed
