...
#define ROE_GRAVITY_CONSTANT 6.6742867e-11          //gravity-constant G
...
#define roe_assert(x) assert((x))
#define ROE_SAFE_DELETE(x)             {if((x)) {delete   (x); (x) = NULL;}}
#define ROE_SAFE_DELETE_ARRAY(x)       {if((x)) {delete[] (x); (x) = NULL;}}
#define ROE_SAFE_DELETE_2ARRAY(x,w)    {if((x)) {for(unsigned long i=0;i<(w);++i) {if((x)[i]) {delete[] (x)[i]; (x)[i]=nullptr;}} delete[] (x); (x)=nullptr;}}
#define ROE_SAFE_DELETE_3ARRAY(x,w,h)  {\
	if((x)) {\
...
#define constexpr const//TODO
constexpr unsigned long roe_pri = 4294967295UL; //primitive restart index
