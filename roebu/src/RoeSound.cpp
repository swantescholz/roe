...
namespace roe {
	...
	void Sound::load(std::string sPath) {
		...
		if (s_mpi == s_mpSound.end()) {
			roe_except("requested sound does not exist: " + sPath, "load");
		}
	...
	void Sound::loadSound (std::string sPath, const float fVolume)	{
		...
		if (s_mpi == s_mpSound.end()) {
			...
			if (sound.chunk) {
				...
				roe_except("sound to load does not exist in path: " + sPath, "loadSound");
			}
		} else {
			roe_except("sound already loaded: " + sPath, "loadSound");
		}
	}
	...
	void Sound::deleteSound	(std::string sPath, bool bCompleteThePath)	{
		...
		s_mpi = s_mpSound.find(sPath);
		if (s_mpi == s_mpSound.end()) {
			roe_except("sound to be deleted does not exist: " + sPath, "deleteSound");
		} else {
