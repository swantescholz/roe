...
namespace roe {
	...
	namespace util {
		...
		bool        isPOT      (unsigned int i);                  //returns wheather i is a power of two
		int         nextPOT    (unsigned int i);                  //returns next power of two
		int         previousPOT(unsigned int i);                  //returns previous power of two
		...
		//math
		//classifications: 1:in front, 0:cut, -1:behind
		...
		void    computeClipPlanes(const Matrix& m, Plane *planes); //left,right,top,bottom,near,far
		...
		template<typename T> void traverseArray2(long w, long h, T func) {
			for (long x = 0L; x < w; ++x) {
				for (long y = 0L; y < h; ++y) {
					func(x,y);
				}
		...
		template<typename T> void traverseArray2(long x0, long y0, long w, long h, T func) {
			for (long x = x0; x < w; ++x) {
				for (long y = y0; y < h; ++y) {
					func(x,y);
				}
		...
		template< typename T, typename... Args>
		void strprintf(std::string& buf, std::string format, const T& head, const Args&... tail)
		{buf = strformatd(format, '%', head, tail...);}//*/
