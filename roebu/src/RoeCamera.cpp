...
namespace roe {
		...
		, dir  (pdir)
	{
		if (flags & CF_CONST_UP_VECTOR) {
			...
			if (flags & CF_DIR_IS_ABS) up = Vector3::cross(Vector3::cross(dir-pos, Vector3(0,1,0)), dir-pos);
			else                       up = Vector3::cross(Vector3::cross(dir    , Vector3(0,1,0)), dir    );
		}
	}
	Camera::Camera(Vector3 ppos, Vector3 pdir)
		: flags(0)
		, pos  (ppos)
		, dir  (pdir)
	{
	...
	: flags(rhs.flags), pos(rhs.pos), dir(rhs.dir), up(rhs.up) {}
		...
		if (flags & CF_CONST_UP_VECTOR) {
			...
			if (flags & CF_DIR_IS_ABS) {
				...
				if (ROE_RAD_TO_DEG(angle) <= 90.0) {
					dir = pos+Matrix::transformNormal(dir-pos, Matrix::rotationAxis(Vector3::cross(up, dir-pos),
				...
				if (ROE_RAD_TO_DEG(angle) <= 90.0f) {
					dir = Matrix::transformNormal(dir, Matrix::rotationAxis(Vector3::cross(up, dir    ),
					                                                        (-radian < angle-variance) ? (radian) : (-angle+variance)));
				} else {
					dir = Matrix::transformNormal(dir, Matrix::rotationAxis(Vector3::cross(up, dir    ),
					                                                       (radian < ROE_PI-angle-variance) ? (radian) : (ROE_PI-angle-variance)));
			...
			if (flags & CF_DIR_IS_ABS) {
				cross = Vector3::cross(up, dir-pos);
