...
namespace roe {
	...
	public:
		...
		Camera(unsigned int pflags, Vector3 ppos, Vector3 pdir, Vector3 pup)
			...
			, dir  (pdir)
			, up   (pup) {}
		Camera(unsigned pflags, Vector3 ppos = Vector3(0,0,0), Vector3 pdir = Vector3(0,0,-1)); //this constructor calculates the up-vector adequately
		Camera(Vector3 ppos = Vector3(0,0,0), Vector3 pdir = Vector3(0,0,-1)); //this constructor calculates the up-vector adequately
		...
		Matrix       getCameraMatrix() const {
			if (isDirAbs()) return Matrix::camera(pos, dir-pos, up);
			return Matrix::camera(pos,dir,up);}
	};
