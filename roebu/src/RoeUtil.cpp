...
namespace roe {
	...
	namespace util {
		...
		bool  isPOT      (unsigned int i) {return i > 0 && (i & (i-1) ) == 0;}
		int   nextPOT    (unsigned int i) {unsigned int j; for(j=1; j<=i; j*=2); return j  ;}
		int   previousPOT(unsigned int i) {if (i<=1) return 1; unsigned int j; for(j=1; j< i; j*=2); return j/2;}
		...
		std::string getFirstNumberOfStringAndErase(std::string &ps) {
			...
			if (tmp.length() == 0) {roe_except("no number found", "getFirstNumberOfStringAndErase");}
			return tmp;
		}
		std::string getFirstStringOfStringAndErase(std::string &ps, char delim) {
			std::string tmp = "";
			size_t pos = ps.find_first_of(delim);
			if (pos == std::string::npos) {roe_except("no string found", "getFirstStringOfStringAndErase");}
		...
		bool getFirstBoolOfStringAndErase(std::string& ps) {
			...
			if (pos1 == std::string::npos && pos2 == std::string::npos) {      //neither true nor false exist
				roe_except("no true or false found ('"+ps+"')", "getFirstBooleanOfStringAndErase");
			}
		...
		std::string addLineNumbersToString(std::string str, const std::string& indent) {
			...
			const int numLength = util::toString(numLines).length();
			...
			for(;;) {
				pos = str.find("\n", pos);
				if (pos == std::string::npos) break;
				str.replace(pos, 1, "\n"+util::addZeros(util::toString(numLines), numLength) + indent);
				++pos;
				++numLines;
			}
			return str;
		...
		std::string toString  (long   i) { char c[40]; sprintf(c, "%ld", i); return std::string(c);}
		...
		std::string toString  (void  *p) { std::stringstream ss; ss<<p; return ss.str();}
		
		
		// *********************************************************
		// MATH
		int classifySphere(const Vector3& center, const float radius, const Plane& plane) {
			//float fDistance = plane.distance(center); //slower(because of normalisation)
			float fDistance = plane.dotCoords(center);
			if(std::abs(fDistance) < radius) return 0;			
			return fDistance > 0.0f ? 1 : -1;
		}
		int classifySphereSq(const Vector3& center, const float radiusSq, const Plane& plane) {
			//float fDistance = plane.distance(center); //slower(because of normalisation)
			float fDistance = plane.dotCoords(center);
			if(fDistance*fDistance < radiusSq) return 0;			
			return fDistance > 0.0f ? 1 : -1;
		}
		int classifyAABox(const Vector3& min, const Vector3& max, const Plane& plane) {
			Vector3 vNearPoint(max);
			Vector3 vFarPoint (min);
			if(plane.n.x > 0.0f) {vNearPoint.x = min.x; vFarPoint.x = max.x;}
			if(plane.n.y > 0.0f) {vNearPoint.y = min.y; vFarPoint.y = max.y;}
			if(plane.n.z > 0.0f) {vNearPoint.z = min.z; vFarPoint.z = max.z;}
			
			if(plane.dotCoords(vNearPoint) > 0.0f) return 1; //front
			if(plane.dotCoords(vFarPoint ) > 0.0f) return 0; //cut
			return -1; //behind
		}
		void computeClipPlanes(const Matrix& mCamera, const Matrix& mProjection, Plane *planes) {
			//compute product:
			computeClipPlanes(mCamera * mProjection, planes);
		}
		void computeClipPlanes(const Matrix& m, Plane *planes) {
			//top
			planes[0].n.x = -(m.m14 + m.m11);
			planes[0].n.y = -(m.m24 + m.m21);
			planes[0].n.z = -(m.m34 + m.m31);
			planes[0].d   = -(m.m44 + m.m41);
			planes[0].normalize();
			
			//right
			planes[1].n.x = -(m.m14 - m.m11);
			planes[1].n.y = -(m.m24 - m.m21);
			planes[1].n.z = -(m.m34 - m.m31);
			planes[1].d   = -(m.m44 - m.m41);
			planes[1].normalize();
			
			//top
			planes[2].n.x = -(m.m14 - m.m12);
			planes[2].n.y = -(m.m24 - m.m22);
			planes[2].n.z = -(m.m34 - m.m32);
			planes[2].d   = -(m.m44 - m.m42);
			planes[2].normalize();
			
			//bottom
			planes[3].n.x = -(m.m14 + m.m12);
			planes[3].n.y = -(m.m24 + m.m22);
			planes[3].n.z = -(m.m34 + m.m32);
			planes[3].d   = -(m.m44 + m.m42);
			planes[3].normalize();
			
			//near
			planes[4].n.x = -(m.m14 + m.m13);
			planes[4].n.y = -(m.m24 + m.m23);
			planes[4].n.z = -(m.m34 + m.m33);
			planes[4].d   = -(m.m44 + m.m43);
			planes[4].normalize();
			
			//far
			planes[5].n.x = -(m.m14 - m.m13);
			planes[5].n.y = -(m.m24 - m.m23);
			planes[5].n.z = -(m.m34 - m.m33);
		...
		bool isPointInSphere(const Vector3& p, const Vector3& center, const double r) {
			return Vector3::length2(p-center) <= r*r;
		}
		double distancePointToRay(const Vector3& p, const Vector3& rayPos, const Vector3& rayDir, bool rayisline) {
			...
			const double dot = Vector3::dot(a,rayDir);
			return std::sqrt(Vector3::dot(a,a)-dot*dot/Vector3::dot(rayDir,rayDir));*/
			const double t = - Vector3::dot((rayPos-p), rayDir) / Vector3::dot(rayDir, rayDir);
			if (rayisline) {
		...
		bool rayHitsSphere(const Vector3& rayPos, const Vector3& rayDir, const Vector3& m, const double r, Vector3 *pvOut, bool rayisline) {
			...
			const double udot = Vector3::dot(rayDir, a);
			const double term = udot*udot - Vector3::dot(rayDir,rayDir) * (Vector3::dot(a,a) - r*r);
			if (term < 0.0) return false;
			if (rayisline == false && pvOut == nullptr) return true;
			const double t = (-udot - std::sqrt(term)) / Vector3::dot(rayDir,rayDir);
			if (rayisline) {
		...
		bool rayHitsPlane(const Vector3& rayPos, const Vector3& rayDir, const Vector3& planePos, const Vector3& up, Vector3 *pvOut, bool rayisline) {
			const double dot1 = Vector3::dot(rayDir, up);
			const double dot2 = Vector3::dot((planePos-rayPos), up);
		...
		bool rayHitsPlane(const Vector3& rayPos, const Vector3& rayDir, const Plane& plane, Vector3 *pvOut, bool rayisline) {
			const double dot = Vector3::dot(rayDir, plane.n);
			if (std::abs(dot) < 0.0001) {
				if (std::abs(Vector3::dot(plane.n, rayPos) + plane.d) < 0.0001) {
					if (pvOut != nullptr)
						*pvOut = rayPos;
					return true;
				}
			...
			if (rayisline == false && pvOut == nullptr) return true;
			const double t = -(Vector3::dot(plane.n, rayPos) + plane.d) / dot;
			if (rayisline) {
				if (t < 0.0 || t > 1.0) return false;
			}
			if (pvOut != nullptr) {
		...
		bool calculateAimingVector(const Vector3& p, const Vector3& u, const Vector3& m, const double c, Vector3 *pvOut) {
			const Vector3 a = p - m;
			const double den   = Vector3::dot(u,u)-c*c;
			const double pHalf = Vector3::dot(a,u)/den;
			const double term  = pHalf*pHalf - Vector3::dot(a,a)/den;
			if (term < 0.0) return false;
			if (pvOut != nullptr) {
		...
		Vector3 calculateCenterOfCircle(const Vector3& v1, const Vector3& v2, const Vector3& v3) {
			const Vector3 vNormal = Vector3::normalize(Vector3::cross(v2-v1, v3-v1));
			const Vector3 vDir1   = Vector3::normalize(Vector3::cross(vNormal, v2-v1));
