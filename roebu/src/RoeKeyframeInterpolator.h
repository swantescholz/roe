...
namespace roe {
	...
	public:
		...
		template<typename U> void addKeyframes(const U& container) {
		...
		virtual void makeConsistentTimeSpacing(bool excludeEnds = false, kfiTime_t fBeginTime = 0.0f, kfiTime_t fEndTime = 1.0f) {
			if (fBeginTime > fEndTime) std::swap(fBeginTime, fEndTime);
		...
		KeyframeL<Type>* findKeyframeBefore(const kfiTime_t fTime) {
			if (fTime <= KeyframeInterpolator<Type>::m_MinTime) return &(m_Keyframes.front());
			if (fTime >= KeyframeInterpolator<Type>::m_MaxTime) return &(m_Keyframes.back ());
			for (m_it1 = m_Keyframes.begin(); m_it1 != m_Keyframes.end(); ++m_it1) {
		...
		virtual Type interpolate(const kfiTime_t fTime) {
			...
			if(pBefore == pAfter) return pBefore->Value;
